...
4. Card com `Aprovado = true` → acionar publicação (hoje: `ig-saves-engine/post_to_instagram.py`, só Instagram; **meta é multirede — Instagram + TikTok primeiro, resto conforme via Metricool no Sprint 6**) → mudar Status pra `Postado`. Enquanto for só Instagram, não prometa publicação em outra rede — reporte a limitação atual se o Lucas pedir TikTok/outra rede antes do Sprint 6 estar pronto.
